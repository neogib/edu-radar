...
@pytest.fixture
def school_type_service(seeded_session: Session) -> SchoolTypeService:
    return SchoolTypeService(seeded_session)


def test_get_all_school_types(school_type_service: SchoolTypeService) -> None:
    school_types = school_type_service.get_school_types()

    assert len(school_types) > 0


def test_get_school_types_by_existing_names(
    ...
    school_types = school_type_service.get_school_types_by_names(query_names)
...
def test_get_school_types_by_empty_names_returns_empty_list(
    ...
    school_types = school_type_service.get_school_types_by_names([])
    assert school_types == []


def test_get_school_type_by_existing_id(
    school_type_service: SchoolTypeService,
) -> None:
    all_school_types = school_type_service.get_school_types()
    assert len(all_school_types) > 0
    ...
    school_type = school_type_service.get_school_type(school_type_id)
