...
from src.data_import.config.excel import EM_FORMULA_PRIORITY, ExamType, ExcelFile
...
class ExcelReader:
    ...
    def _parse_file_metadata(
        ...
        if year_match:
            return _FileMetadata(path=file_path, year=int(year_match.group("year")), priority=0)
